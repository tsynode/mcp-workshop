...
def process_pending_tool_uses():
    ...
    # Failsafe: If we've been processing for too long, force continue
    max_tool_processing_iterations = 5
    current_iteration = 0
    
    # Continue processing tool uses until there are none left or we hit an error
    while conversation_manager.has_pending_tool_uses() and current_iteration < max_tool_processing_iterations:
        current_iteration += 1
        logger.info(f"Processing tool iteration {current_iteration}")
        ...
        if not tool_use:
            ...
            # Remove this ID from pending to avoid infinite loop
            conversation_manager.pending_tool_uses.remove(tool_use_id)
            continue
        ...
        with st.status(f"Executing tool {tool_name}...", expanded=False) as status:
            try:
                ...
                if result_message is None:
                    ...
                    # But don't break - continue to next tool
                else:
    ...
    if not conversation_manager.has_pending_tool_uses() and conversation_manager.is_processing_tools():
        ...
        # If we hit the max iterations or have other issues, force continue
        if current_iteration >= max_tool_processing_iterations and conversation_manager.has_pending_tool_uses():
            logger.warning(f"Reached maximum tool processing iterations ({max_tool_processing_iterations}), forcing continuation")
...
if mode == "Manual MCP Tool Tester":
    ...
    if tool_count == 0:
        ...
        # Recovery button for stuck conversations
        if st.session_state.conversation_manager.has_pending_tool_uses():
            if st.button("🔄 Force Continue (if stuck)"):
